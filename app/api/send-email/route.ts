...
export async function POST(req: NextRequest) {
	try {
		...
		const subject = isAr ? "نتائج اختبار الشخصية" : "Your Big Five Personality Results";
		...
		const scoresHtml = result.scores.map((s: any) => {
			...
			return `<tr><td style="padding:10px 0;border-bottom:1px solid #e8f0f7;">
        <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
          <span style="font-weight:700;color:${info.color};font-size:14px;">${isAr ? info.ar : info.en}</span>
          <span style="font-size:13px;color:${info.color};font-weight:600;">${s.score}/50 (${s.percent}%)</span>
        </div>
...
<div style="max-width:560px;margin:0 auto;padding:32px 16px;">
  ...
  <div style="background:#3b9dd4;border-radius:20px 20px 0 0;padding:28px;text-align:center;">
    <p style="color:rgba(255,255,255,0.85);margin:0 0 8px;font-size:14px;">${isAr ? `مرحبا ${name}` : `Hello ${name}`}</p>
  ...
  <div style="background:white;border-radius:0 0 20px 20px;padding:24px;margin-bottom:16px;">
  ...
  <div style="background:white;border-radius:20px;padding:20px;text-align:center;margin-bottom:16px;">
  ...
  <p style="text-align:center;font-size:11px;color:#8ab4d4;">big5personalities.duckdns.org</p>
</div></body></html>`;
